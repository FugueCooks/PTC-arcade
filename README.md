...
The Kingdom Hearts and Grand Theft Auto: San Andreas cabinets are enabled as isolated local-file prototypes using the official experimental Play!.js WebAssembly build. They accept legally dumped ISO, CSO, CHD, ISZ, BIN, and ELF files and require no external BIOS. The selected file remains local to the visitor's browser and is passed directly to the emulator without being uploaded. Kingdom Hearts was verified to boot at approximately 53 FPS on the development PC. GTA San Andreas v3.00 reached its title screen at approximately 23 FPS during the initial browser test, so performance and later-game compatibility remain experimental. The 8.5 GB God of War image remained at 0 FPS in the same browser test, so that cabinet and the two unassigned PS2 slots remain disabled pending compatibility work. The runtime snapshot and upstream license live under `emulators/play/`; PS1 and N64 continue to use the unchanged EmulatorJS player.
