...
Five GameCube-ready cabinets line the front wall of the main arcade, opposite the prize counter. They are assigned, in order, to The Legend of Zelda: The Wind Waker, The Legend of Zelda: Twilight Princess, Pikmin, Super Smash Bros. Melee, and Super Mario Sunshine. The approved `.ciso` images are tracked in `deploy/remote-gamecube-assets.json` and may be hosted outside Git, but both their game records and cabinets intentionally remain disabled until a vetted browser-compatible Dolphin/WebAssembly runtime is integrated. The official Dolphin project currently distributes native Windows, macOS, Linux, and Android builds rather than an embeddable browser build. This prevents the UI from presenting a Play button that cannot work.
