...
The Kingdom Hearts, Grand Theft Auto: San Andreas, and Dragon Ball Z: Budokai Tenkaichi 3 cabinets are enabled as isolated local-file prototypes using the official experimental Play!.js WebAssembly build. They accept legally dumped ISO, CSO, CHD, ISZ, BIN, and ELF files and require no external BIOS. The selected file remains local to the visitor's browser and is passed directly to the emulator without being uploaded. Kingdom Hearts was verified to boot at approximately 53 FPS on the development PC. GTA San Andreas v3.00 reached its title screen at approximately 23 FPS, and Dragon Ball Z: Budokai Tenkaichi 3 reached its title screen at approximately 29 FPS during initial browser testing, so performance and later-game compatibility remain experimental. The 8.5 GB God of War image remained at 0 FPS in the same browser test, so that cabinet and the final unassigned PS2 slot remain disabled pending compatibility work. The runtime snapshot and upstream license live under `emulators/play/`; PS1 and N64 continue to use the unchanged EmulatorJS player.

The cabinet waits for the Play! iframe to announce that its local file loader is ready before handing off the selected disc image, and Play! acknowledges the handoff before the cabinet clears its loading timeout. Exiting resets the file selector so the same image can be selected again. This handshake keeps multi-gigabyte local files from being lost during iframe startup; it does not upload or host the image.
