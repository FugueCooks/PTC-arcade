...
Five GameCube cabinets line the front wall of the main arcade, opposite the prize counter. They run the pinned experimental Gecko WebGPU core and are assigned, in order, to The Legend of Zelda: The Wind Waker, The Legend of Zelda: Twilight Princess, Pikmin, Super Smash Bros. Melee, and Super Mario Sunshine. Their approved `.rvz` images are tracked in `deploy/remote-gamecube-assets.json`, stored in R2 rather than Git, and delivered with public CORS and byte-range support. The patched core enables RVZ parsing, streams the download into one pre-sized WebAssembly disc buffer, raises Gecko's web-memory ceiling to 2 GiB, and bounds the Winit canvas to the emulator viewport. Pikmin and Super Mario Sunshine were verified to initialize locally without loader or WebGPU canvas errors before the cabinets were enabled.

GameCube support remains experimental. Gecko currently retains the complete compressed RVZ image in browser memory, so desktop Chromium with WebGPU and substantial free memory is required; mobile and memory-constrained devices may fail, and the 1.43 GB Melee image is especially demanding. The five source images are not bundled into the frontend or sent through multiplayer. Exiting removes the emulator iframe and releases its browser-side disc buffer.
