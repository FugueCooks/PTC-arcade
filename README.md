...
The PS2 gallery is accessible through the centered doorway in the rear of the PlayStation room. That doorway is enforced consistently by local collision and authoritative Node/Cloudflare movement validation, while the solid wall sections on either side remain impassable. The Xbox and GameCube expansions remain temporarily closed with visible caution-tape construction walls and room-specific **Room Under Construction.** notices.

Five GameCube cabinets line the front wall of the main arcade, opposite the prize counter. They run the pinned experimental Gecko WebGPU core and are assigned, in order, to The Legend of Zelda: The Wind Waker, The Legend of Zelda: Twilight Princess, Pikmin, Super Smash Bros. Melee, and Super Mario Sunshine. Their approved `.rvz` images are tracked in `assets/runtime/gamecube-digests.json`, stored in R2 rather than Git, and delivered with public CORS and byte-range support. The patched core enables RVZ parsing, streams the first download into one pre-sized WebAssembly disc buffer, and simultaneously retains a size-validated copy in origin-private browser storage when quota permits. Later launches use that local copy and avoid downloading the multi-gigabyte image again. Gecko still has to copy the complete compressed image into WebAssembly memory before booting because its current disc API is not range-backed. The runtime raises Gecko's web-memory ceiling to 2 GiB and bounds the Winit canvas to the emulator viewport. Pikmin and Super Mario Sunshine were verified to initialize locally without loader or WebGPU canvas errors before the cabinets were enabled.
...
Kingdom Hearts was verified to reach its title screen at approximately 53 FPS on the development PC. Progress beyond the title screen remains experimental in the Play! browser build; the upstream project documents browser-specific JIT limitations for games that load additional EE modules. A black opening movie may sometimes be skipped with Enter (PS2 Start), but this is not a guarantee that the remainder of the game is browser-compatible. GTA San Andreas v3.00 reached its title screen at approximately 23 FPS, and Dragon Ball Z: Budokai Tenkaichi 3 reached its title screen at approximately 29 FPS during initial browser testing, so performance and later-game compatibility remain experimental. While any emulator is open, the outer Three.js renderer, avatar animation updates, and ambient world effects suspend so the emulator receives the available CPU/GPU budget. Hosted PS2 discs use a bounded 160 MB in-memory range cache, adaptive two- or four-chunk read-ahead (disabled in data-saver mode), and a bounded origin-private persistent range cache. Previously visited disc regions are therefore reused across launches without requiring a complete multi-gigabyte download. The production asset host is `assets.ptcarcade.fun`; its Worker stores each bounded disc range as an independent edge-cache object so large images are not excluded by the CDN's whole-object size limit. Range reads are validated, retried with a timeout, and surfaced as a clear cabinet error rather than leaving the emulator silently stalled. The 8.5 GB God of War image remained at 0 FPS in the same browser test, so that cabinet and the final unassigned PS2 slot remain disabled pending compatibility work. The runtime snapshot and upstream license live under `emulators/play/`; PS1 and N64 continue to use the unchanged EmulatorJS player.
...
Touch-capable devices receive an in-frame PS2 controller overlay. The movement pad drives both the left analog stick and D-pad so it works across title menus and gameplay, while dedicated controls expose the right analog camera, face buttons, shoulder buttons, Start, and Select. Multi-touch button holds dispatch Play!'s native keyboard mappings directly to its focused canvas; desktop keyboard and physical-controller behavior is unchanged.

Approaching a PS2 cabinet asks the browser to prefetch only the approximately 2.7 MB Play! runtime, never the multi-gigabyte game image. Automatic range caching is capped at 128 MB on devices reporting 4 GB or less of memory and 512 MB elsewhere for each game. Supported Chromium browsers also show an explicit **CACHE GAME LOCALLY** control for players who want the complete disc image available after one download. After checking available quota and requesting persistent storage, that control streams the selected hosted image into origin-private file-system storage with visible progress. Complete cached files are size-validated and preferred on future launches; aborted or incomplete files are removed, and unsupported browsers continue using R2 range streaming. Cached copies belong to that browser profile and origin and can be removed through the browser's site-data controls.
