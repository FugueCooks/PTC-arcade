...
void test('the platform endpoint reports honest capability', async () => {
  ...
  try {
    ...
    assert.equal(body.data.cabinetDefinitions, 82);
...
void test('pagination walks the whole collection exactly once', async () => {
  ...
  try {
    ...
    do {
      ...
      assert.equal(page.meta.total, 82);
      cursor = page.meta.nextCursor;
    } while (cursor !== null);

    assert.equal(seen.length, 82);
    assert.equal(new Set(seen).size, 82, 'no cabinet may appear twice across pages');
  } finally {
    await api.close();
  }
});
...
void test('filtering and sorting narrow through indexes', async () => {
  ...
  try {
    ...
    assert.equal(zone.meta.total, 9);
