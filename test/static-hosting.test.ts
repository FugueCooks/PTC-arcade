...
void test('runtime config preserves local game URLs when no CDN is configured', () => {
  assert.deepEqual(publicRuntimeConfig({}), { gameAssetBaseUrl: '', biosAssetUrl: '' });
});
...
void test('runtime config serialization cannot inject a script tag', () => {
  const script = runtimeConfigScript({ gameAssetBaseUrl: 'https://cdn.example.com/<script>', biosAssetUrl: '' });
