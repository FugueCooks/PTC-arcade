...
const OPEN_DOOR_Z_WEST = [-25.2, -8, 8]; // the bare room behind the 25.2 doorway is deleted and sealed
...
function isInsideWorld(x: number, z: number): boolean {
  ...
  if (x >= -119.7 && x <= -42.7 && z >= -63 && z <= 11.8) return true;
...
function violatesSocialLayout(fromX: number, fromZ: number, toX: number, toZ: number): boolean {
  ...
  // The Silent Hill district is sealed while it is rebuilt: no crossing is a door crossing.
  const throughSilentDoor = (_x: number) => false;
