export class LightingManager {
  constructor(scene, config) {
    this.scene = scene; this.config = config; this.lights = []; this.emissives = []; this.accentLights = []; this.targetMultiplier = 1; this.currentMultiplier = 1; this.eventUntil = 0; this.eventType = null; this.nextUpdateAt = 0;
  ...
  update(now, delta) {
    ...
    this.lights.forEach(({ light, base }, index) => { if (!light.visible) return; const flicker = index % 5 === 0 ? .96 + Math.sin(now * .0027 + index) * .04 : 1; light.intensity = base * this.currentMultiplier * eventFactor * flicker; });
