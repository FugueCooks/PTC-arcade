...
import { JukeboxClient } from './jukebox-client.js';
...
export class WorldManager {
  static async create(arcade, socket) {
    const response = await fetch('assets/world/config.json?v=phase6-2'); if (!response.ok) throw new Error('World configuration could not load.');
  ...
  apply(state) { if (this.state && state.revision < this.state.revision) return; this.state = state; this.environment.apply(state); this.lighting.apply(state); this.audio.setActivity(state.activityLevel); this.jukebox.apply(state.jukebox); const theme = this.config.themes.find((candidate) => candidate.id === state.themeId); document.querySelector('#world-status').textContent = `${theme?.name ?? state.themeId} · ${state.activityLevel}`.toUpperCase(); }
