...
export class WorldManager {
  ...
  update(now) { if (this.disposed) return; requestAnimationFrame(this.frame); if (now < this.nextFrameAt) return; this.nextFrameAt = now + this.updateIntervalMs; const delta = Math.min(.05, (now - this.lastFrameAt) / 1000); this.lastFrameAt = now; this.interactions.update(now); this.audio.update(now); this.lighting.update(now, delta); this.particles.update(delta); }
  dispose() { this.disposed = true; clearTimeout(this.announcementTimer); this.audio.dispose(); this.lighting.dispose(); this.environment.dispose(); this.interactions.dispose(); this.particles.dispose(); }
}
