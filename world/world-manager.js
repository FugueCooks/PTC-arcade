import { AudioManager } from './audio-manager.js?v=arcade-rows-6';
import { EnvironmentManager } from './environment-manager.js?v=arcade-rows-6';
...
export class WorldManager {
  ...
  apply(state) { if (this.state && state.revision < this.state.revision) return; this.state = state; this.environment.apply(state); this.lighting.apply(state); this.audio.setActivity(state.activityLevel); const theme = this.config.themes.find((candidate) => candidate.id === state.themeId); document.querySelector('#world-status').textContent = `${theme?.name ?? state.themeId} · ${state.activityLevel}`.toUpperCase(); }
  interact(object) { this.audio.click(); const messages = { 'prize-counter': 'PRIZE COUNTER // SHOP COMING LATER' }; this.announce({ text: messages[object.type] ?? `${object.name} // READY`, audioCue: 'notice' }); }
  ...
  update(now) { if (this.disposed) return; requestAnimationFrame(this.frame); if (this.suspended || now < this.nextFrameAt) return; this.nextFrameAt = now + this.updateIntervalMs; const delta = Math.min(.05, (now - this.lastFrameAt) / 1000); this.lastFrameAt = now; this.interactions.update(now); this.audio.update(now); this.lighting.update(now, delta); this.particles.update(delta); }
  dispose() { this.disposed = true; window.removeEventListener('arcade:emulator-mode-changed', this.onEmulatorModeChanged); clearTimeout(this.announcementTimer); this.audio.dispose(); this.lighting.dispose(); this.environment.dispose(); this.interactions.dispose(); this.particles.dispose(); }
}
