...
function getOptimizedGltfLoader(){if(!optimizedGltfLoaderPromise)optimizedGltfLoaderPromise=Promise.all([import('https://cdn.jsdelivr.net/npm/three@0.160.1/examples/jsm/loaders/GLTFLoader.js'),import('https://cdn.jsdelivr.net/npm/three@0.160.1/examples/jsm/libs/meshopt_decoder.module.js')]).then(([{GLTFLoader},{MeshoptDecoder}])=>new GLTFLoader().setMeshoptDecoder(MeshoptDecoder));return optimizedGltfLoaderPromise;}
...
let nextLightCull=0;
function updateNearbyLights(now){if(now<nextLightCull)return;nextLightCull=now+250;for(const cabinet of cabinets){const visible=cabinet.g.position.distanceToSquared(playerPosition)<100;cabinet.renderLights.forEach(light=>{light.visible=visible});}for(const light of managedSceneLights){const dx=light.position.x-playerPosition.x,dz=light.position.z-playerPosition.z;light.visible=dx*dx+dz*dz<196;}const centerDistanceSq=playerPosition.x*playerPosition.x+playerPosition.z*playerPosition.z;gangsterPepeLight.visible=centerDistanceSq<36;counterDisplayLight.visible=centerDistanceSq<64;const prizeVisible=playerPosition.distanceToSquared(prizeDisplay.position)<196;prizeDisplayLights.forEach(light=>{light.visible=prizeVisible});}
