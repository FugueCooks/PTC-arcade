...
const playerPosition = new THREE.Vector3(0, 1.65, 11), followOffset = new THREE.Vector3(), cameraTarget = new THREE.Vector3(), lookDirection = new THREE.Vector3();
...
const point = (color,x,y,z,intensity=6) => { const l=new THREE.PointLight(color,intensity,10,2);l.position.set(x,y,z);scene.add(l);return l };
...
function makeCabinet(id,name,x,z,hue,isCrash=false,isGex=false){
  ...
  scene.add(g);cabinets.push({id,g,name,type:id.toUpperCase(),screen,hue,statusLight,status:'syncing',occupiedByDisplayName:null,enabled:true});
}
...
function openMachine(c){
  ...
  if(c.hostedGame) playButton.textContent=`PLAY ${c.gameName.toUpperCase()}`;
...
const hostedPsxBios='assets/bios/SCPH1001.BIN';
...
function launchEmulator(gameFile){
  ...
  cvs.style.display='none';stage.style.display='grid';stage.style.placeItems='center';stage.style.color='#36f9f6';stage.style.fontFamily='monospace';stage.style.letterSpacing='.12em';host.textContent=`LOADING ${(activeCabinet?.gameName||'ARCADE GAME').toUpperCase()}...`;
  ...
  clearTimeout(emulatorLoadTimer);emulatorLoadTimer=setTimeout(()=>{if(activeCabinet){closeMachine();showCabinetMessage('EMULATOR LOAD TIMED OUT.')}},20000);
}
...
function updateFollowCamera(){
  ...
  if(followed){
    ...
    followOffset.set(0,2.3,4.8).applyAxisAngle(new THREE.Vector3(0,1,0),heading);
  ...
  followOffset.set(0,2.15+pitch*2.1,4.55).applyAxisAngle(new THREE.Vector3(0,1,0),yaw);
...
window.arcadeMultiplayer={scene,getCamera:()=>camera,getCanvas:()=>renderer.domElement,getLocalTransform:()=>({position:{x:playerPosition.x,y:playerPosition.y,z:playerPosition.z},rotationY:yaw}),getLocalAnimationState:()=>localAnimationState,isFirstPerson:()=>cameraMode==='first-person',getCameraMode:()=>cameraMode,isFollowingPlayer:()=>Boolean(socialFollowProvider),followPlayer:provider=>{socialFollowProvider=provider},clearPlayerFollow:()=>{socialFollowProvider=null},applyAuthoritativeTransform:({position,rotationY},strength=.12)=>{playerPosition.lerp(new THREE.Vector3(position.x,position.y,position.z),strength);const difference=Math.atan2(Math.sin(rotationY-yaw),Math.cos(rotationY-yaw));yaw+=difference*strength;},setCabinetState,setCabinetStates,showCabinetMessage,beginCabinetSession,forceCloseCabinetSession};
function tick(){requestAnimationFrame(tick);const d=Math.min(clock.getDelta(),.05);animatedMixers.forEach(mixer=>mixer.update(d));drawPrizeLed(performance.now());if(locked){const v=new THREE.Vector3((keys.KeyD?1:0)-(keys.KeyA?1:0),0,(keys.KeyS?1:0)-(keys.KeyW?1:0));localAnimationState=v.length()?'walk':'idle';if(v.length()){v.normalize().multiplyScalar(d*5).applyAxisAngle(new THREE.Vector3(0,1,0),yaw);const previousX=playerPosition.x;playerPosition.add(v);resolvePartitionWallCollisions(previousX);playerPosition.x=Math.max(-27,Math.min(27,playerPosition.x));playerPosition.z=Math.max(-16,Math.min(16,playerPosition.z))}near=null;let md=2.25;cabinets.forEach(c=>{const dist=c.g.position.distanceTo(playerPosition);if(dist<md){near=c;md=dist}});updateCabinetPrompt()}else{localAnimationState=activeCabinet?'interact':'idle';if(performance.now()>=cabinetMessageUntil)prompt.classList.remove('active')}updateFollowCamera();game();renderer.render(scene,camera)}tick();
