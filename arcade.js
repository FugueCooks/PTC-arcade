import { GAMEPAD_AXES, GAMEPAD_BUTTONS, buttonPressed, DEFAULT_DEAD_ZONE as GAMEPAD_DEAD_ZONE, gamepadHasActivity, pickGamepad, readDpad, readStick } from './emulators/gamepad-mapping.js?v=trading-1';
...
const coolPanelMaterial=new THREE.MeshStandardMaterial({color:0xa8e6cd,emissive:0x2fb488,emissiveIntensity:.6,roughness:.55});
const haloTexture=(()=>{
...
const coolHaloMaterial=new THREE.MeshBasicMaterial({map:haloTexture,color:0x2fb488,transparent:true,opacity:.34,depthWrite:false,blending:THREE.AdditiveBlending});
...
box(18,.09,.08,0xc98a12,0,4.78,TOP_BAND_MIN_Z+.23,.85);box(18,.12,.08,0x0c2119,0,.1,TOP_BAND_MIN_Z+.23,.55);
...
function buildPartitionWall(wallX,accent,segments){
  for(const [centerZ,depth] of segments){
    ...
    box(.42,.08,depth,accent,wallX,4.86,centerZ,.9);box(.42,.1,depth,0x0c2119,wallX,.08,centerZ,.45);
  }
}
...
buildPartitionWall(PLAYSTATION_WALL_X,0xc98a12,PARTITION_WALL_SEGMENTS_WEST);
buildPartitionWall(N64_WALL_X,0x16c784,PARTITION_WALL_SEGMENTS_EAST);
/**
...
hangMuralWalls([
    {file:'ff-room-mural.webp?v=trading-1',span:32,at:new THREE.Vector3(-5.4,2.5,-66.94),
    ...
    {file:'ff-room-mural-2.webp?v=trading-1',span:16,at:new THREE.Vector3(10.54,2.5,-59),
    ...
    {file:'ff-room-mural-3.webp?v=trading-1',span:16,at:new THREE.Vector3(-21.34,2.5,-59),
...
let templeOfTimeStarted=false,templeStarted=false,templeMount=null,templeSettle=false;
...
function installTempleOfTime(){
  // Guarded the way the castle is. templeOfTimeStarted belongs to the callers:
  // the proximity gate tests it, and the preload step SETS it and then calls
  // regardless. So whenever a player spawned within the gate's reach, the
  // temple was fetched twice -- 11MB downloaded and decoded for nothing, on
  // every single load. The installer owns whether it has already run.
  if(templeStarted)return;
  templeStarted=true;
  void (async()=>{try{
    const loader=await getOptimizedGltfLoader();
...
function installPikomat(){
  ...
  void (async()=>{try{
    const loader=await getOptimizedGltfLoader();
    loader.load('assets/models/props/pikomat.glb?v=trading-1',gltf=>{
...
function installPeachsCastle(){
  ...
  void (async()=>{try{
    ...
    loader.load('assets/models/mario/peachs-castle-1f.glb?v=castle-model-1',gltf=>{
      ...
      void getOptimizedGltfLoader().then(pipeLoader=>pipeLoader.load('assets/models/mario/warp-pipe.glb?v=trading-1',pipeGltf=>{
...
function buildSilentHillBlock(){
  ...
  // The district's buildings and cast are deleted: 6.3MB of models and some
  // three hundred meshes for a street behind construction tape. The roads and
  // kerbs stay -- they are a handful of planes, and they are what the room
  // reads as from its doorway.
...
lightRoom(MEGAMAN_ROOM_CENTER_X,MEGAMAN_ROOM_CENTER_Z,MEGAMAN_ROOM_WIDTH,MEGAMAN_ROOM_DEPTH,0x16c784);
...
const SIDE_ROOM_ACCENTS=[0xea3943,0xf0b90b,0x16c784,0x3ddc97];
for(const roomX of [-ANNEX_ROOM_CENTER_X,ANNEX_ROOM_CENTER_X]){
  ...
  SIDE_ROOM_Z.forEach((centerZ,index)=>{
    ...
    for(let z=at-6;z<=at+6;z+=4){
      ...
      box(ROOM_SPAN-.5,.035,.055,0x2f7d5f,roomX,4.65,z,.8);
    }
...
lightRoom(10.8,TOP_BAND_CENTER_Z,64.8,ROOM_DEPTH,0xf0b90b);
...
for(const bandZ of [-25.2,-8.4,8.4,25.2])lightRoom(0,bandZ,HALL_HALF_WIDTH*2,ROOM_DEPTH,bandZ<0?0xf0b90b:0x16c784);
...
box(SHELL_HALF_WIDTH-ROOM_DOOR_HALF_WIDTH,5,.3,0x11182c,(SHELL_HALF_WIDTH+ROOM_DOOR_HALF_WIDTH)/2,2.5,TOURNAMENT_ROOM_DOOR_Z,.06);
for(let x=-40;x<=40;x+=4)box(3.82,.055,.06,0x2f7d5f,x,4.66,TOURNAMENT_ROOM_BACK_Z-.19,.75);
lightRoom(0,TOURNAMENT_ROOM_CENTER_Z,TOURNAMENT_ROOM_WIDTH,TOURNAMENT_ROOM_DEPTH,0xf0b90b);
...
function makeModelCabinet(id,name,x,z,hue,system,model){
  ...
  const statusMaterial=new THREE.MeshStandardMaterial({color:0x50ff9a,emissive:0x50ff9a,emissiveIntensity:2.4});
  // The status light sits a metre to the side of the machine, level with the
  // marquee plate's end -- which reads as part of the plate, and reads as a
  // speck hanging in mid-air without one. Every machine that trades its plate
  // for floor art hides it: the mat names the game, and an in-use light that
  // floats beside nothing is just a bright dot in the dark. The mesh is still
  // built and still recoloured by setCabinetState, so nothing downstream has
  // to know it is not on screen.
  const statusLight=new THREE.Mesh(cabinetGeometry.statusLight,statusMaterial);statusLight.position.set(1.05,model.statusY??(model.plateY??3.4)-.02,.1);statusLight.visible=!model.noPlate;g.add(statusLight);
...
/**
 * The hall's machines stand in one unbroken row along the east wall.
 *
 * They were split across both partition walls, which left two half-empty runs
 * and no clear front to the room. Fifteen machines at the house spacing,
 * centred on the wall's solid stretch: it runs z -12 to 33.6, with the Pokemon
 * storefront's opening north of that and the tournament hall's front wall
 * south. Console order along the wall — PlayStation, PS2, Nintendo 64,
 * GameCube — so the room reads as one shelf you walk the length of.
 *
 * The two tables below feed the four row builders unchanged. Each builder
 * indexes its own table and skips machines that moved to themed rooms over the
 * life of the project, so both tables carry those gaps; what matters is that
 * the slots still in use land in order along the row.
 */
const HALL_ROW_Z=Array.from({length:15},(_,index)=>-5.3+index*ARCADE_ROW_SPACING);
const hallSlot=index=>({x:WALL_ROW_X,z:HALL_ROW_Z[index],rotation:-Math.PI/2});
// PlayStation holds 0-6 and PS2 7-9. The zeroed entries are the deleted GOD OF
// WAR and PS2 // READY 05 plates: the PS2 builder indexes slots 7 through 11
// and skips those two, so the three live machines read from 8, 9 and 10.
const FOYER_WEST=[0,1,2,3,4,5,6,0,7,8,9,0].map(hallSlot);
// Nintendo 64 holds 10-12 and GameCube 13-14. The zeroed entries are Pokemon
// Snap, Ocarina of Time, Super Mario 64, Wind Waker, Twilight Princess and
// Sunshine — every one of them stands in a themed room now.
const FOYER_EAST=[0,0,10,11,0,12,0,0,0,13,14,0].map(hallSlot);
...
const POKEMON_MACHINE_ROW=[
  ...
  ['nds-cabinet-03','ds',32.87,0xd9d9e6,{noPlate:true,statusY:2.31,mat:{file:'pokemon-platinum-mat.webp',w:2.4}}],
  ['nds-cabinet-04','ds',36.12,0xffcf6b,{noPlate:true,statusY:2.31,mat:{file:'pokemon-heartgold-mat.webp',w:2.4}}],
  ['nds-cabinet-05','ds',39.37,0xc0c0d0,{noPlate:true,statusY:2.31,mat:{file:'pokemon-soulsilver-mat.webp',w:2.4}}],
  ['nds-cabinet-06','ds',42.62,0x4a4a5f,{noPlate:true,statusY:2.31,mat:{file:'pokemon-black-mat.webp',w:2.4}}],
  ['nds-cabinet-07','ds',45.87,0xf0f0f5,{noPlate:true,statusY:2.31,mat:{file:'pokemon-white-mat.webp',w:2.4}}],
  ['nds-cabinet-08','ds',49.12,0x5f5f74,{noPlate:true,statusY:2.31,mat:{file:'pokemon-black-2-mat.webp',w:2.4}}],
  ['nds-cabinet-09','ds',52.37,0xfafaff,{noPlate:true,statusY:2.31,mat:{file:'pokemon-white-2-mat.webp',w:2.4}}]
];
...
const ZELDA_MACHINE_MODELS={
  handheld:{file:'assets/models/zelda/zelda-gba-cabinet.glb?v=trading-1',scale:1.55,lift:.496,modelRotY:-Math.PI/2,plateY:1.74,plinthScale:1.15,statusY:1.72},
  ds:{file:'assets/models/zelda/zelda-ds-cabinet.glb?v=trading-1',scale:.1,lift:-.005,plateY:1.86,plinthScale:1.3,statusY:1.84},
  gamecube:{file:'assets/models/zelda/zelda-gamecube-cabinet.glb?v=trading-1',scale:.22,lift:.004,plateY:1.74,plinthScale:1.25,statusY:1.72},
  n64:{file:'assets/models/zelda/zelda-n64-cabinet.glb?v=trading-1',scale:.85,lift:.211,plateY:1.5,plinthScale:1.2,statusY:1.48},
  nes:{file:'assets/models/zelda/zelda-nes-cabinet.glb?v=trading-1',scale:3.7,lift:.159,plateY:1.44,plinthScale:1.1,statusY:1.42}
};
...
const MARIO_MACHINE_MODELS={
  ...
  smb:{noPlate:true,file:'assets/models/mario/mario-smb-arcade.glb?v=trading-1',scale:.0726,lift:.018,offsetZ:-.196,plateY:2.62,statusY:2.6,plinthScale:1.2},
  bros:{noPlate:true,file:'assets/models/mario/mario-bros-arcade.glb?v=trading-1',scale:1.3583,lift:-.071,offsetZ:-.135,plateY:2.62,statusY:2.6,plinthScale:1.05},
  smb3:{noPlate:true,file:'assets/models/mario/mario-smb3-arcade.glb?v=trading-1',scale:1.4985,lift:1.35,plateY:2.62,statusY:2.6,plinthScale:1.15},
  // and the console shells the Zelda room already brought in
  n64:{noPlate:true,file:'assets/models/zelda/zelda-n64-cabinet.glb?v=trading-1',scale:.85,lift:.211,plateY:1.5,statusY:1.48,plinthScale:1.2},
  nes:{noPlate:true,file:'assets/models/zelda/zelda-nes-cabinet.glb?v=trading-1',scale:3.7,lift:.159,plateY:1.44,statusY:1.42,plinthScale:1.1},
  gamecube:{noPlate:true,file:'assets/models/zelda/zelda-gamecube-cabinet.glb?v=trading-1',scale:.22,lift:.004,plateY:1.74,statusY:1.72,plinthScale:1.25}
};
...
const SEGA_MACHINE_MODELS={
  genesis:{file:'assets/models/sega/sega-genesis.glb?v=trading-1',scale:3.7,lift:0,plateY:1.44,statusY:1.42,plinthScale:1.1},
  dreamcast:{file:'assets/models/sega/sega-dreamcast.glb?v=trading-1',scale:3.2,lift:-.051,plateY:1.44,statusY:1.42,plinthScale:1.1},
  ps2:{file:'assets/models/sega/sega-dreamcast.glb?v=trading-1',scale:0,lift:0,plateY:2.62,statusY:2.6,plinthScale:1.4}
};
...
for(const [cabinetId,kind,rowX,gameOrLabel,hue] of SONIC_GARDEN_ROW){
  ...
  if(kind==='ps2'){
    ...
    // Named as Dreamcast so the prompt can say so. Nothing emulates a
    // Dreamcast in a browser: neither EmulatorJS's cores nor libretro's
    // emscripten build carries flycast, reicast or redream, so these two
    // stand as displays rather than machines that pretend to boot.
    if(kind==='dreamcast')Object.assign(cabinets[cabinets.length-1],
      {system:'dreamcast',enabled:false,status:'disabled',disabledReason:'display-only'});
  }
  configureHostedCabinet(cabinetId);
...
const inlayMaterial=new THREE.MeshBasicMaterial({color:0x16c784,transparent:true,opacity:.42,depthWrite:false,blending:THREE.AdditiveBlending});
const inlayWarmMaterial=new THREE.MeshBasicMaterial({color:0x7fe3b8,transparent:true,opacity:.17,depthWrite:false,blending:THREE.AdditiveBlending});
 ...
 */
const thresholdGlow=new THREE.MeshBasicMaterial({color:0x3ddc97,transparent:true,opacity:.34,depthWrite:false,blending:THREE.AdditiveBlending});
const jambMaterial=new THREE.MeshStandardMaterial({color:0x11201a,emissive:0x16c784,emissiveIntensity:1.35,roughness:.3,metalness:.55});
...
const haloRings=[];
for(const [radius,y,tube,color,speed,tilt] of [[2.35,3.05,.045,0x16c784,.16,.05],[3.05,3.7,.038,0xea3943,-.11,-.07],[3.75,4.32,.03,0xf0b90b,.075,.04]]){
...
function preloadSceneModels(onProgress){
  ...
  // Only what a player can see from the spawn, or walks into within seconds of
  // it. Peach's Castle, the Pokemon Center and the arena are minutes away
  // through their own doorways and load on approach the way they always could
  // -- keeping them here bought a hitch-free walk into rooms most players never
  // open, and charged every player the download to get in the door.
  const steps=[
...
point(0x16c784,-2.6,2.3,0,2);point(0xea3943,2.6,2.3,0,2);
...
function warmStreamingDisc(cabinet){
  ...
  import('./emulators/disc-range-cache.js?v=trading-1')
