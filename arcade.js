...
function stopEmulator(){
  ...
  activeEmulatorFrame=null;pendingPs2File=null;
...
let emulatorObjectUrls=[],emulatorLoadTimer,activeEmulatorFrame=null,pendingPs2File=null;
...
function launchEmulator(gameFile){
  ...
  const player=document.createElement('iframe');player.title=`${gameName} player`;player.allow='autoplay; fullscreen';player.src=isPs2?'emulators/play/index.html?v=ps2-handoff-2':`player.html?core=${encodeURIComponent(core)}&game=${encodeURIComponent(gameUrl)}&bios=${encodeURIComponent(biosUrl)}&name=${encodeURIComponent(gameName)}&id=${gameId}`;player.style.cssText='border:0;width:100%;height:100%;background:#02030a';player.onerror=()=>{showCabinetMessage('EMULATOR COULD NOT LOAD.');closeMachine()};activeEmulatorFrame=player;pendingPs2File=isPs2&&gameFile instanceof File?gameFile:null;host.replaceChildren(player);
