...
// Temporary construction barriers extend the PlayStation partition across
// both end passages. The PS2 room remains intact behind them for later work.
...
for(const z of [-15.25,15.25]){const barrier=new THREE.Group();barrier.position.set(PLAYSTATION_WALL_X,0,z);const panel=new THREE.Mesh(new THREE.PlaneGeometry(3.1,2.65),new THREE.MeshBasicMaterial({map:constructionTexture,side:THREE.DoubleSide}));panel.position.set(.205,1.48,0);panel.rotation.y=Math.PI/2;barrier.add(panel);for(const edgeZ of [-1.56,1.56]){const post=new THREE.Mesh(new THREE.BoxGeometry(.22,3.05,.18),new THREE.MeshStandardMaterial({color:0x161616,emissive:0x4b3600,emissiveIntensity:.35,metalness:.78,roughness:.28}));post.position.set(.18,1.52,edgeZ);barrier.add(post);const beacon=new THREE.PointLight(0xffb000,1.7,2.8,2);beacon.position.set(.38,2.9,edgeZ);barrier.add(beacon);managedSceneLights.push(beacon)}scene.add(barrier);constructionBarriers.push(barrier)}
...
function closeMachine(notifyServer=true){const closing=activeCabinet;ps2CacheController?.abort();ps2CacheController=null;if(['psx','n64','ps2'].includes(closing?.system))stopEmulator();modal.style.display='none';modal.setAttribute('aria-hidden','true');activeCabinet=null;if(notifyServer&&closing)window.dispatchEvent(new CustomEvent('arcade:cabinet-session-ended',{detail:{cabinetId:closing.id}}));renderer.domElement.requestPointerLock()}
...
let emulatorObjectUrls=[],emulatorLoadTimer,activeEmulatorFrame=null,pendingPs2Source=null;
...
function nearPs2ConstructionBarrier(){return Math.abs(playerPosition.x-PLAYSTATION_WALL_X)<3.2&&Math.abs(playerPosition.z)>12.35}
function updateConstructionPrompt(){prompt.classList.add('active');prompt.querySelector('b').textContent='CAUTION';prompt.querySelector('span').textContent='PS2 Room Under Construction.'}
...
function resolvePartitionWallCollisions(previousX){
  for(const wallX of [PLAYSTATION_WALL_X,N64_WALL_X]){
    const isPlaystationWall=wallX===PLAYSTATION_WALL_X;
    const wallHalfLength=isPlaystationWall?16.8:PARTITION_WALL_HALF_LENGTH;
    ...
    if(previousX<wallX&&playerPosition.x>leftFace){if(!isPlaystationWall||Math.abs(playerPosition.z)<=PARTITION_WALL_HALF_LENGTH+PLAYER_COLLISION_RADIUS)playerPosition.x=leftFace}
